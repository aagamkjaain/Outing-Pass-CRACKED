...
-- Context helpers for non-JWT warden auth
CREATE OR REPLACE FUNCTION set_user_context(user_name text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  PERFORM set_config('app.user_name', COALESCE(user_name, ''), TRUE);
END;
$$;
...
STABLE
AS $$
  SELECT nullif(current_setting('app.user_name', true), '');
$$;
...
-- 8. System users (wardens) can view outing requests for assigned hostels
CREATE POLICY system_user_view_outing_requests ON outing_requests
    FOR SELECT
    USING (
      EXISTS (
        SELECT 1 FROM system_users
        WHERE system_users.username = get_user_name()
          AND outing_requests.hostel_name = ANY(system_users.hostels)
      )
    );

-- 9. System users (wardens) can update outing requests for assigned hostels
...
-- 16. System users (wardens) can view student info for assigned hostels
CREATE POLICY system_user_view_student_info ON student_info
    ...
    USING (
      EXISTS (
        SELECT 1 FROM system_users
        WHERE system_users.username = get_user_name()
          AND student_info.hostel_name = ANY(system_users.hostels)
      )
    );
...
-- 17. Allow login by username (kept broad to support client-side login lookups)
CREATE POLICY allow_login_by_username ON system_users
    FOR SELECT
    USING (true);

-- 17b. Wardens may read only their own row explicitly (optional hardening)
CREATE POLICY warden_view_own_record ON system_users
    FOR SELECT
    USING (username = get_user_name());
...
-- 19. Wardens can view bans (read-only)
CREATE POLICY warden_view_ban_students ON ban_students
    FOR SELECT
    USING (
      EXISTS (
        SELECT 1 FROM system_users
