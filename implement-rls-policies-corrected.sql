-- =====================================================
-- ROW LEVEL SECURITY (RLS) POLICIES IMPLEMENTATION - CORRECTED
...
AS $$
  SELECT COALESCE(
    (SELECT hostels FROM wardens WHERE email = get_user_email()),
    ARRAY[]::TEXT[]
  );
$$;
...
-- Superadmins can read their own admin record only
CREATE POLICY "superadmins_read_own_admin" ON admins
  FOR SELECT
  TO authenticated
...
-- Superadmins can read all warden records (for management)
...
-- Superadmins can read all student info (for management)
...
-- Superadmins can read all outing requests (for management)
...
-- =====================================================
-- BAN_STUDENTS TABLE POLICIES
-- =====================================================

-- Superadmins can read all bans (for management)
CREATE POLICY "superadmins_read_all_ban_students" ON ban_students
  FOR SELECT
  TO authenticated
  USING (is_superadmin());

-- Wardens can read bans for students in their assigned hostels
CREATE POLICY "wardens_read_assigned_hostel_ban_students" ON ban_students
  FOR SELECT
  TO authenticated
  USING (
    is_warden() AND 
    EXISTS (
      ...
      WHERE si.student_email = ban_students.student_email 
      AND si.hostel_name = ANY(get_warden_hostels())
    )
  );

-- Students can read their own bans
CREATE POLICY "students_read_own_ban_students" ON ban_students
  FOR SELECT
  TO authenticated
  USING (student_email = get_user_email());

-- Superadmins can insert bans
CREATE POLICY "superadmins_insert_ban_students" ON ban_students
  FOR INSERT
  TO authenticated
  WITH CHECK (is_superadmin());

-- Wardens can insert bans for students in their assigned hostels
CREATE POLICY "wardens_insert_assigned_hostel_ban_students" ON ban_students
  FOR INSERT
  TO authenticated
  WITH CHECK (
    is_warden() AND 
...
CREATE POLICY "superadmins_update_ban_students" ON ban_students
  FOR UPDATE
  TO authenticated
  USING (is_superadmin())
  WITH CHECK (is_superadmin());
...
CREATE POLICY "wardens_update_assigned_hostel_ban_students" ON ban_students
  FOR UPDATE
  TO authenticated
  USING (
    is_warden() AND 
...
CREATE POLICY "superadmins_delete_ban_students" ON ban_students
  FOR DELETE
  TO authenticated
  USING (is_superadmin());

-- Wardens can delete bans for students in their assigned hostels
CREATE POLICY "wardens_delete_assigned_hostel_ban_students" ON ban_students
  FOR DELETE
  TO authenticated
  USING (
    is_warden() AND 
...
AND tablename IN ('admins', 'wardens', 'arch_gate', 'student_info', 'outing_requests', 'ban_students')
ORDER BY tablename;
